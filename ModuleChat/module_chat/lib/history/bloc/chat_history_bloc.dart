...
class ChatHistoryBloc extends Cubit<ChatHistoryState> {
  ...
  Future<void> takeChatHistory() async {
    try {
      if (state is ChatHistoryStateInit) {
        var data = await getIt.get<ApiDataSource>().listHistory(pageNum: pageNumHistory);
        if (data == null || data.length < maxItemHistory) {
          emit(ChatHistoryStateSuccessful().cloneWrite(
              listHistory: data, hasReachedEnd: true));
        } else {
          emit(ChatHistoryStateSuccessful().cloneWrite(
              listHistory: data, hasReachedEnd: true));
          pageNumHistory++;
  ...
  Future<Void> logOut({BuildContext context,String userName,String donViID,String token}) async{
